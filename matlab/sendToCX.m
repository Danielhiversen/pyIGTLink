function sendToCX(input_data) 
    ...
    persistent ctr;
     ...
     if isempty(IGTLink)
        'Start server'
        if count(py.sys.path,'C:\Users\danielho\Documents\pyIGTLink\') == 0
            insert(py.sys.path,int32(0),'C:\Users\danielho\Documents\pyIGTLink\');
        end
    ...
    packet = IGTLink.ImageMessageMatlab(reshape(imdata',1,dim(1)*dim(2)),[dim(2), dim(1)], spacing);
    %send image message
